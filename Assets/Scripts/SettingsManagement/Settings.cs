...
namespace ARKitect.SettingsManagement
{
    ...
    {
        ISettingsRepository[] _settingsRepositories;
        ...
        /// <summary>
        /// Creates a new Settings instance with a <see cref="UserSettingsRepository"/> and <see cref="ProjectSettingsRepository"/>.
        /// </summary>
        /// <param name="project">The project name</param>
        /// <param name="settingsFileName">The name of the settings file. The default value is `Settings`.</param>
        public Settings(string project, string settingsFileName = "Settings")
        {
            _settingsRepositories = new ISettingsRepository[]
            {
                new ProjectSettingsRepository(project, settingsFileName),
        ...
        {
            _settingsRepositories = repositories.ToArray();
        }
        ...
        {
            foreach (var repo in _settingsRepositories)
                if (repo.Scope == scope)
                    return repo;
            return null;
        }
        ...
        public void Set<T>(string key, T value, SettingsScope scope = SettingsScope.Project)
        {
            if (scope == SettingsScope.Project)
                Set<T, ProjectSettingsRepository>(key, value);
        ...
        {
            if (scope == SettingsScope.Project)
                Set<T, ProjectSettingsRepository>(key, value, repositoryName);
        ...
        public void Set<T, K>(string key, T value, string repositoryName = null) where K : ISettingsRepository
        {
            ...
            if (!foundScopeRepository)
                Logger.LogWarning($"No repository with type {typeof(K)} found.");
        }
        ...
        {
            if (scope == SettingsScope.Project)
                return Get<T, ProjectSettingsRepository>(key, fallback);
        ...
        {
            if (scope == SettingsScope.Project)
                return Get<T, ProjectSettingsRepository>(key, fallback, repositoryName);
        ...
        {
            if (scope == SettingsScope.Project)
                return ContainsKey<T, ProjectSettingsRepository>(key);
        ...
        {
            if (scope == SettingsScope.Project)
                return ContainsKey<T, ProjectSettingsRepository>(key, repositoryName);
        ...
        {
            if (scope == SettingsScope.Project)
                DeleteKey<T, ProjectSettingsRepository>(key);
        ...
        {
            if (scope == SettingsScope.Project)
                DeleteKey<T, ProjectSettingsRepository>(key, repositoryName);
