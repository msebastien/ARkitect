...
using PBAutoUnwrapSettings = UnityEngine.ProBuilder.AutoUnwrapSettings;
...
namespace ARKitect.Geometry
{
    ...
    public struct AutoUnwrapSettings
    {
        ...
        /// </summary>
        /// <param name="unwrapSettings">The settings to copy to the new instance.</param>
        public AutoUnwrapSettings(PBAutoUnwrapSettings unwrapSettings)
        {
            ...
            m_Fill = (Fill)unwrapSettings.fill;
            m_Scale = unwrapSettings.scale;
            m_Offset = unwrapSettings.offset;
            m_Rotation = unwrapSettings.rotation;
            m_Anchor = (Anchor)unwrapSettings.anchor;
        }

        public void CopyTo(PBAutoUnwrapSettings unwrapSettings)
        {
            unwrapSettings.useWorldSpace = m_UseWorldSpace;
            unwrapSettings.flipU = m_FlipU;
            unwrapSettings.flipV = m_FlipV;
            unwrapSettings.swapUV = m_SwapUV;
            unwrapSettings.fill = (PBAutoUnwrapSettings.Fill)m_Fill;
            unwrapSettings.scale = m_Scale;
            unwrapSettings.offset = m_Offset;
            unwrapSettings.rotation = m_Rotation;
            unwrapSettings.anchor = (PBAutoUnwrapSettings.Anchor)m_Anchor;
        }
